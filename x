if shared.Nocturne.Memory.Settings.IsPrecisionActive == true then
    local Memory
    ...
    while task.wait(1) do
       local minMemory = shared.Nocturne.Memory.Configuration.Start
       local maxMemory = shared.Nocturne.Memory.Configuration.End
    ...
    local config = shared.Nocturne.TriggerBot
    ...
    local function isWithinViewEra(position)
        ...
        local fovHeight = shared.Nocturne.ViewEra.Vertical * 100
        local fovWidth = shared.Nocturne.ViewEra.Horizontal * 100
    ...
    local AssistReach = shared.Nocturne.PointAssist.AssistReach
    ...
    Mouse.KeyDown:Connect(function(Key)
        ...
        if key == shared.Nocturne.ToggleKey:lower() then
            if shared.Nocturne.PointAssist.IsPrecisionActive then
                if IsTargeting then
        ...
        if key == shared.Nocturne.CancelKey:lower() then
            IsTargeting = false
            TargetPlayer = nil
        end
    end)
        ...
        if IsTargeting and TargetPlayer and TargetPlayer.Character then
            ...
            if bodyPart then
                ...
                if distanceToTarget <= math.sqrt(
                    ...
                    if shared.Nocturne.PointAssist.Resolver then
                        local humanoid = TargetPlayer.Character:FindFirstChildOfClass("Humanoid")
                        if humanoid then
                            local moveDirection = humanoid.MoveDirection
                            predictedPosition = bodyPart.Position + (moveDirection * Vector3.new(
                                shared.Nocturne.PointAssist.Compensation.CompensationX,
                                shared.Nocturne.PointAssist.Compensation.CompensationY,
                                shared.Nocturne.PointAssist.Compensation.CompensationZ
                            ))
                        end
                    else
                    ...
                    if predictedPosition then
                        ...
                        Camera.CFrame = Camera.CFrame:Lerp(DesiredCFrame, shared.Nocturne.PointAssist.AssistStabilization)
                    end
    
                    if shared.Nocturne['Silent Targeting'].IsAssistActive and IsTargeting and TargetPlayer.Character:FindFirstChild("Humanoid") then
                        ...
                        if success and velocity then
                            Replicated_Storage[RemoteEvent]:FireServer(Argument, closestPoint + velocity * Vector3.new(
                                shared.Nocturne['Silent Targeting'].TimeAlteration.TimeShiftX, 
                                shared.Nocturne['Silent Targeting'].TimeAlteration.TimeShiftY, 
                                shared.Nocturne['Silent Targeting'].TimeAlteration.TimeShiftZ
                            ))
                        end
                    end
                end
    ...
    local function updateFov()
        local settings = shared.Nocturne['Silent Targeting'].TargetingRange
        clearFovParts()
    ...
    local function isPartInFovAndVisible(part)
        if not shared.Nocturne.PointAssist.IsPrecisionActive or not IsTargeting or not TargetPlayer then
            return false
        end
    
        
        local dynamicFovSize = calculateFov(shared.Nocturne['Silent Targeting'].TargetingRange.BoundaryX, 
                                             shared.Nocturne['Silent Targeting'].TargetingRange.BoundaryY, 
                                             shared.Nocturne['Silent Targeting'].TargetingRange.BoundaryZ)
    ...
    local function isPartVisible(part)
        if not shared.Nocturne['Silent Targeting'].WallCheck then 
            return true
        end
    ...
    local OldTimeAlteration = shared.Nocturne['Silent Targeting'].TimeAlteration
    ...
    local function GetVelocity(player, part)
        if player and player.Character then
            ...
            local distortionX = shared.Nocturne['Silent Targeting'].TimeAlteration.TimeShiftX
            local distortionY = shared.Nocturne['Silent Targeting'].TimeAlteration.TimeShiftY
            local distortionZ = shared.Nocturne['Silent Targeting'].TimeAlteration.TimeShiftZ
            ...
            if adjustedVelocity.Y < -30 and shared.Nocturne['Silent Targeting'].Resolver then
                shared.Nocturne['Silent Targeting'].TimeAlteration = { TimeShiftX = 0, TimeShiftY = 0, TimeShiftZ = 0 }
                return adjustedVelocity
            elseif adjustedVelocity.Magnitude > 50 and shared.Nocturne['Silent Targeting'].Resolver then
                return player.Character:FindFirstChild("Humanoid").MoveDirection * 16 * distortionX
            else
                shared.Nocturne['Silent Targeting'].TimeAlteration = OldTimeAlteration
                return adjustedVelocity
            end
        end
    ...
    if input.KeyCode == Enum.KeyCode[shared.Nocturne.ToggleKey:upper()] and shared.Nocturne.PointAssist.Method == "hold" then
    ...
    RunService.RenderStepped:Connect(function()
        ...
        if shared.Nocturne['Silent Targeting'].IsPrecisionActive and IsTargeting then
            if TargetPlayer then
                if TargetPlayer.Character then
                    ...
                    if success3 and closestPart and closestPoint then
                        ...
                        local TimeAlterationX = shared.Nocturne['Silent Targeting'].TimeAlteration.TimeShiftX
                        local TimeAlterationY = shared.Nocturne['Silent Targeting'].TimeAlteration.TimeShiftY
                        local TimeAlterationZ = shared.Nocturne['Silent Targeting'].TimeAlteration.TimeShiftZ
    ...
    task.spawn(function()
        while task.wait(0.1) do
            if shared.Nocturne['Silent Targeting'].IsPrecisionActive then
